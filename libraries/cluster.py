...
###  Here, a matrix of distances is progressively memoized and 
# saved into a pickle file, in case multiple rounds are run (give task name)

def finalClusters( designs, numClust, dirPath, verbose=False):
	# Hardcoded path to Blosum 80  # KALE should integrate this into workspace
	path2matrix='scripts/BLOSUM80'

	### Read in and store substition matrix ###
	firstRowFlag=0
	matchDict={}
	r=0
	flag=0
	...
	## Score is SSE (Sum of sequare distances)
	...
	def kmedoids(numClust, designs, memo):
		...
		## Pick initial cluster centers (random), remove from main list, add self to cluster
	...
	return clustering
 	...
 	pic.dump(memo, open( os.path.join( dirPath, 'memoCluster.pkl' ), 'wb'  ))
